...
for ax, n_i in zip(axes, sample_sizes):
    ax.hist(sim_results[n_i], bins=30, edgecolor="white")
...
fig.suptitle("Sampling Distribution of the Difference in Means", fontsize=14)
fig.tight_layout()
plt.show()
```
...
4. Once we know the null distribution, we can compute a p-value.
