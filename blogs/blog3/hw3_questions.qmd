...
predicted_y0 = np.exp(X0_df.to_numpy() @ beta_hat)
predicted_y1 = np.exp(X1_df.to_numpy() @ beta_hat)

average_effect = np.mean(predicted_y1 - predicted_y0)

effect_table = pd.DataFrame({
    "Quantity": ["Average effect of Blueprinty customer status"],
    "Estimate": [average_effect]
})

effect_table.style.format({"Estimate": "{:.10f}"})
```

The estimated average treatment effect is essentially zero. In practical terms, this means that after controlling for differences in firm age and regional location, the model predicts almost no difference in patent output between Blueprinty customers and non customers.

This result is interesting because the earlier coefficient interpretation suggested that customer status was positively associated with patent activity. However, when the model is used to generate firm level counterfactual predictions and average them across the entire dataset, the overall effect becomes negligible.

One possible explanation is that the customer effect is relatively small compared to the overall variation in patent outcomes across firms. Another possibility is that the nonlinear structure of the Poisson model causes the average marginal effect to differ substantially from the raw coefficient interpretation.

Substantively, the findings suggest that once observable firm characteristics are taken into account, Blueprinty’s software does not appear to meaningfully change predicted patent counts on average in this sample.

At the same time, this remains an observational analysis rather than a randomized experiment. Even though the regression controls for observable differences such as age and region, unobserved characteristics, such as firm-level innovation culture, R&D investment, or management quality, could still influence both software adoption and patent output.

As a result, the analysis does not establish a strong causal effect of Blueprinty’s software on patent success. Instead, it suggests that the relationship between Blueprinty customer status and patent outcomes is weaker than the raw comparisons initially implied.
